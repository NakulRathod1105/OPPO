import React, { useState, useEffect } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { useDispatch } from "react-redux";
import { myAction } from "../Redux/Action";

export default function AudioDetails() {
  const { id } = useParams();
  const [audio, setAudio] = useState(null);
  const dispatch = useDispatch();
  const navigate = useNavigate();

  useEffect(() => {
    fetch(`http://localhost:3000/Audio/${id}`)
      .then((response) => response.json())
      .then((data) => setAudio(data))
      .catch((error) => console.error("Error fetching audio data:", error));
  }, [id]);

  if (!audio) {
    return <div>Loading...</div>;
  }

  function cart() {
    dispatch(myAction(audio));
    navigate("/cart");
  }

  return (
    <div className="container audio-details p-4">
      <div className="row">
        <div className="col-md-6 col-sm-6 col-12">
          <img
            src={audio.image}
            className="img-fluid phone-image"
            alt={audio.name}
          />
        </div>


        <div className="col-md-6 col-sm-6 col-12 mt-4 mt-md-0">
          <p className="text-success fs-5">One-day delivery available</p>
          <h2 className="phone-title mb-3">{audio.name}</h2>
          <button className="btn btn-primary rounded-5 p-2 mb-3">
            Point Benefits
          </button>
          <p className="phone-price fs-3 mb-3">
            <span className="current-price">₹{audio.price}</span>
            <del className="original-price ms-2 text-secondary">
              ₹{audio.originalPrice}
            </del>{" "}
            (incl. of all taxes)
          </p>
          <p className="phone-description fs-4 mb-4">{audio.description}</p>

          <button className="btn btn-dark btn-lg" onClick={cart}>
            Add To Cart
          </button>
        </div>
      </div>
    </div>
  );
}
